...
func TestGetWeatherDataHandler(t *testing.T) {
	t.Run("Should return weather data related to Joinville, SC, BR successfully", func(t *testing.T) {
		defer gock.Off()
		...
		expected := "{\"weather\":[{\"main\":\"Clouds\",\"description\":\"broken clouds\"}],\"base\":\"stations\",\"main\":{\"temp\":26.04,\"feels_like\":26.04,\"temp_min\":24.96,\"temp_max\":28.4,\"humidity\":81},\"wind\":{\"speed\":1.03,\"deg\":260},\"name\":\"Joinville\"}\n"
		if rr.Body.String() != expected {
			t.Errorf("handler returned unexpected body: got %v want %v", rr.Body.String(), expected)
		}
	})

	t.Run("Should return error message if failed to get lat lon data", func(t *testing.T) {
		defer gock.Off()
		...
		expected := "{\"error\":\"Failed to get latitude and longitude data\"}\n"
		if rr.Body.String() != expected {
			t.Errorf("handler returned unexpected body: got %v want %v", rr.Body.String(), expected)
		}
	})
